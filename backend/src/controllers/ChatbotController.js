...
class ChatbotController {
    ...
    async handleUserInput(req, res) {
        ...
        const containsfashionKeywords = (input) => {
            const lowerInput = input.toLowerCase();
            return fashionKeywords.some(keyword => lowerInput.includes(keyword));
        }

        if (!containsfashionKeywords(input)) {
            res.status(500).send({
                message: "Please ask something about fashion only"
            })
        }
